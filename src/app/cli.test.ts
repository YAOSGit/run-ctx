...
vi.mock('../utils/resolver/index.js', async (importOriginal) => {
	...
	return {
		...mod,
		resolveAlias: vi.fn((_aliases: unknown, args: string[]) => {
...
describe('CLI execution', () => {
	...
	it('T6: --dry-run delegates to resolveAlias to ensure consistency with normal execution', () => {
		...
		vi.mocked(resolveModule.resolveAlias).mockReturnValueOnce(
			fakeMatch as unknown as ReturnType<typeof resolveModule.resolveAlias>,
		);
		vi.mocked(matcherModule.findBestMatch).mockReturnValueOnce({
			command: 'echo nested',
			score: 1,
			rule,
		} as unknown as ReturnType<typeof matcherModule.findBestMatch>);
